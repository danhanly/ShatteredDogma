...
export const WorshipperStat: React.FC<WorshipperStatProps> = ({ 
    ...
    // Priority 2: Interaction Glow (Type specific)
    else if (isGlowing) {
        switch(type) {
    ...
    // Priority 3: Passive Influence Glow (Purple)
    ...
    <div className="flex items-center">
        <div 
            ...
            data-clickable="true"
            className={`group relative flex w-[75px] cursor-pointer flex-col items-center justify-center gap-1 rounded-lg border ${borderClass} bg-black/60 p-2 backdrop-blur-sm transition-all duration-300 hover:bg-black/80 hover:scale-105 active:scale-95 sm:w-[100px] sm:min-w-[100px] ${opacityClass} ${scaleClass} ${glowShadow} ${isInfluenceShaking ? 'animate-shake' : ''}`}
        >
            ...
            <Icon className={`h-4 w-4 ${isStarved ? 'text-red-500' : iconColor} sm:h-6 w-6 transition-colors`} />
            <span className={`hidden font-serif text-xs ${isStarved ? 'text-red-400' : textColor} sm:block transition-colors`}>{type}</span>
            <span className={`block font-serif text-[10px] ${isStarved ? 'text-red-400' : textColor} sm:hidden transition-colors`}>{type.slice(0, 3)}</span>
            <span className={`font-mono text-xs font-bold sm:text-lg ${isStarved ? 'text-red-500' : 'text-white'} transition-colors`}>{formatNumber(count)}</span>
        </div>
